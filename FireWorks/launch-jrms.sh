...
function display_help {
    ...
    echo "  delete_wf    Delete a workflow from the launchpad. Requires an additional argument for the workflow ID."
...
case "$1" in
    ...
    delete_wf)
        ...
        python /fw/jrm_launcher/gen_wf.py delete_wf --fw_id "$2"
        ;;
    delete_ports)
        if [ -z "$2" ] || [ -z "$3" ]; then
