...
function display_help {
    ...
    echo "Commands:"
    echo "  add_wf       Add a workflow to the launchpad. Requires environment variables SITE and CONFIG_FILE to be set."
...
case "$1" in
    add_wf)
        if [ -z "$2" ] || [ -z "$3" ]; then
            echo "Please provide a configuration file and site to add a workflow."
            exit 1
        fi
        python /fw/gen_wf.py add_wf --config_file "$2" --site "$3"
        ;;
    get_wf)
