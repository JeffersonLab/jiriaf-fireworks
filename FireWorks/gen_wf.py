...
from components.launch import *
from components import ssh
...
class MainJrmManager:
    ...
    def get_jrm_manager(self):
        if self.site_name == "perlmutter":
            return PerlmutterJrmManager()
        elif self.site_name == "ornl":
            return OrnlJrmManager()
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--site", required=True, choices=["perlmutter", "ornl"], help="Site name (perlmutter or ornl)")
    ...
    if args.action == "add_wf":
        jrm_manager = MainJrmManager(args.site)
        jrm_manager.add_jrm()
