...
if {$is_reversed eq "false"} {
    set first_hop_cmd "ssh -L *:$port:localhost:$port jlabtsai@flux.op.ccs.ornl.gov"
    set second_hop_cmd "ssh -L *:$port:localhost:$port jlabtsai@128.219.141.17"
} else {
    set first_hop_cmd "ssh -R $port:localhost:$port jlabtsai@flux.op.ccs.ornl.gov"
    set second_hop_cmd "ssh -R $port:localhost:$port jlabtsai@128.219.141.17"
}
